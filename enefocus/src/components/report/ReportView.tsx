...
import { Share2, Copy, CheckCircle, Video, Clock, Download, FileImage, Zap } from 'lucide-react';
import { useStore } from '@/lib/store';
import { formatDuration, getToday, subjectColorHex, subjectColorMap } from '@/lib/utils';
import { useDailyReport } from '@/hooks/useDailyReport';

export function ReportView() {
  const { todos, studySessions, subjects, todoRecords, lifeLogs, dailyReports, addDailyReport, updateDailyReport, user } = useStore();
  const [selectedDate, setSelectedDate] = useState(getToday());
  const [shareVis, setShareVis] = useState<'private' | 'link'>('link');
  const [copied, setCopied] = useState(false);
  const [exporting, setExporting] = useState(false);
  const reportRef = useRef<HTMLDivElement>(null);
  const { generateReport } = useDailyReport();
  ...
  const savedReport = (dailyReports || []).find((r) => r.reportDate === selectedDate);
  const shareId = savedReport?.publicShareId || `report-${selectedDate}-${user?.id}`;
  ...
  function handleGenerateReport() {
    const existing = (dailyReports || []).find((r) => r.reportDate === selectedDate);
    if (existing) {
      updateDailyReport(existing.id, { visibility: shareVis });
      alert('리포트가 업데이트되었습니다!');
    } else {
      const report = generateReport(selectedDate);
      addDailyReport({ ...report, visibility: shareVis });
      alert('리포트가 생성되었습니다!');
    }
  }
  ...
  async function handleExportImage() {
    ...
    try {
      ...
      link.download = `enefocus-report-${selectedDate}.png`;
  ...
  async function handleExportPdf() {
    ...
    try {
      ...
      pdf.save(`enefocus-report-${selectedDate}.pdf`);
    } catch (e) {
      ...
      <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            disabled={exporting}
            className="flex items-center gap-1.5 px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded-xl text-sm font-medium text-gray-700 transition-colors disabled:opacity-50"
            title="이미지로 저장"
          >
            <FileImage size={16} />
          ...
          <div className="space-y-4">
            <div className="bg-black text-white rounded-2xl p-5">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-white text-black rounded-full py-2.5 text-sm font-bold hover:bg-gray-100 transition-colors mb-3"
              >
                <Zap size={16} />
                {savedReport ? '리포트 업데이트' : '리포트 생성'}
              </button>

              {savedReport && shareVis === 'link' && (
                <div className="space-y-2">
            ...
            {(dailyReports || []).length > 0 && (
              <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                <h3 className="font-bold text-gray-900 mb-3 text-sm">저장된 리포트</h3>
                <div className="space-y-1.5">
                  {[...(dailyReports || [])].reverse().slice(0, 7).map((r) => (
                    <button
                      key={r.id}
                      onClick={() => setSelectedDate(r.reportDate)}
                      className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm transition-colors ${
                        r.reportDate === selectedDate ? 'bg-black text-white' : 'bg-gray-50 text-gray-700 hover:bg-gray-100'
                      }`}
                    >
                      <span className="font-medium">{r.reportDate}</span>
                      <span className={`text-xs ${r.reportDate === selectedDate ? 'text-gray-400' : 'text-gray-400'}`}>
                        {formatDuration(r.totalStudySeconds)}
                      </span>
                    </button>
                  ))}
                </div>
